...
/**
 * Exact membership, deliberately not origins.has().
 *
 * Reverting this one line to `origins.has(origin)` is what turns a favicon
 * cache into an SSRF proxy on any deployment that ever set ALLOWED_ORIGINS=*.
 */
export const allowed = (origin) => origins.list().includes(origin);

export function refuse(origin) {
  let u;
  ...
  if (!allowed(origin)) return 'that origin is not on the allowlist';
  return null;
}
 ...
 */
export async function iconFor(origin, { now = Date.now() } = {}) {
  const no = refuse(origin);
