 ...
 */
// One organisation's allowlist. There is no global one any more: every
// origin belongs to an org (docs/AUTH.md §10), and so does every icon lookup.
const space = origins.forOrg('icons-check');
const listed = () => space.list();

if (refuse('https://not-on-the-list.example', listed())) ok('an origin not on the allowlist is refused');
else bad('an origin not on the allowlist is refused');

const wild = spawnSync(process.execPath, ['-e', `
  import('./origins.js').then(async (O) => {
    const { refuse } = await import('./icons.js');
    // The list an org with ALLOWED_ORIGINS=* actually holds. has() would say
    // yes to anything; membership must not.
    const list = O.forOrg('wildcard-test').list();
    console.log(refuse('https://evil.example', list) ? 'REFUSED' : 'FETCHED');
  });
...
space.add(IMDS);
try {
  if (listed().includes(IMDS)) {
    if (refuse(IMDS, listed())) ok('and is refused EVEN WHEN ALLOWLISTED', 'the ordering that matters on EC2');
...
} finally { space.remove(IMDS); }
if (!listed().includes(IMDS)) ok('  (test cleaned up after itself)');
...
for (const [bad_, why] of [
  ...
  if (refuse(bad_, listed())) ok(`refuses ${why}`);
...
try {
  site.mode = 'huge';
  const huge = await iconFor(ORIGIN, listed(), { now: fresh() }).catch(() => 'threw');
  ...
  site.mode = 'html';
  const html = await iconFor(ORIGIN, listed(), { now: fresh() }).catch(() => 'threw');
  ...
  site.mode = 'away';
  other.hits = 0;
  const away = await iconFor(ORIGIN, listed(), { now: fresh() }).catch(() => 'threw');
  ...
  site.hits = 0;
  const at = fresh();
  const one = await iconFor(ORIGIN, listed(), { now: at });
  ...
  const two = await iconFor(ORIGIN, listed(), { now: at + 1000 });   // same window: must be cached
  ...
  const gone = `${ORIGIN}`;
  const miss = fresh();
  await iconFor(gone, listed(), { now: miss });
  const before = site.hits;
  await iconFor(gone, listed(), { now: miss + 1000 });   // same window: the miss is remembered
